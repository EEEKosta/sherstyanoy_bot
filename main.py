...
async def cmd_start(message: types.Message):
	await message.answer(
		'Привет, я Шерстяной 😸. Я здесь что бы дарить улыбку! Но все не так просто, что бы получить картинку с котейкой продолжи фразу "Три кота - *** ***", каждый раз новая картинка. Если нужна подсказка напиши - /help'
	)

@dp.message_handler(commands=['help'])
async def cmd_start(message: types.Message):
	await message.answer(
		'Каждый раз когда ты пишешь мне кодовую фразу, я присылаю тебе разные картинки. Подсказка здесь https://youtu.be/tCL1PzE9RhE?t=12. Или напиши моему хозяину @E_Kostya'
	)
